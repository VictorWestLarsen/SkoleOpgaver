...
namespace UnitTestProject1
{
    ...
    public class UnitTest1
    {
        ...
        [TestMethod]
        public void TestSortWithDelegateParameter()
        {
            MyLinkedList<ClubMember> list = new MyLinkedList<ClubMember>();
            ClubMember x = new ClubMember() { FirstName = "Uma", LastName = "Sarrell" };
            ClubMember y = new ClubMember() { FirstName = "George", LastName = "Mish" };

            list.Insert(p2);  // FirstName : Trev,       LastName : Quail
            list.Insert(p4);  // FirstName : Hyacinthie, LastName : Mish
            list.Insert(p23); // FirstName : Solomon,    LastName : Sarrell
            list.Insert(p9);  // FirstName : Olimpia,    LastName : Foulsham
            list.Insert(p5);  // FirstName : Jarib,      LastName : Boustred
            list.Insert(x);   // FirstName : Uma,        LastName : Sarrell
            list.Insert(y);   // FirstName : George,     LastName : Mish

            list.Sort(// Anonymous delegate: Sort by LastName, if equal: sort after firstName
               delegate (ClubMember c1, ClubMember c2)
               {
            ...
            Assert.AreEqual(p5, list.ItemAt(0)); // FirstName : Jarib,      LastName : Boustred
            Assert.AreEqual(p9, list.ItemAt(1)); // FirstName : Olimpia,    LastName : Foulsham
            Assert.AreEqual(y, list.ItemAt(2));  // FirstName : George,     LastName : Mish
            Assert.AreEqual(p4, list.ItemAt(3)); // FirstName : Hyacinthie, LastName : Mish
            Assert.AreEqual(p2, list.ItemAt(4)); // FirstName : Trev,       LastName : Quail
            Assert.AreEqual(p23, list.ItemAt(5));// FirstName : Solomon,    LastName : Sarrell
            Assert.AreEqual(x, list.ItemAt(6));  // FirstName : Uma,        LastName : Sarrell
        }
